...
#data processing
data_train = pd.read_csv('~/nlp/stackoverflow_dataset_glove_model/data_train.csv').to_numpy()
data_test = pd.read_csv("~/nlp/stackoverflow_dataset_glove_model/data_test.csv").to_numpy()
...
d = json.load(f)
weight = torch.zeros(len(vocab) + 1, 300)
for k, v in vocab.items():
    if k in d.keys():
        weight[v] = torch.from_numpy(np.array(d[k]))
f.close()
...
class mlp_classifier(nn.Module):
  def __init__(self, vocab):
    ...
    self.emb = nn.Embedding.from_pretrained(weight, padding_idx = len(vocab))

    self.layer2 = nn.Linear(300, 500)
    self.layer3 = nn.Linear(500, 4)

  def forward(self, xb):
    h = self.emb(xb) # 3d, (batch_size X max_seq_len X 300)
    h = torch.mean(h, dim=1) # 2d, (batch_size X 300)
    h = F.relu(h)
    h = self.layer2(h) # pre-activation outputs are called 'logits',  (batch_size X 4)
    h = F.relu(h)
    h = self.layer3(h)
...
model = mlp_classifier(vocab).to(device)
